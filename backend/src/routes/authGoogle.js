...
// Google rotates keys ~every 24 h. Cache for 1 h so verification is a local
// CPU operation instead of an HTTP round-trip on every sign-in.
let _googleKeys = null;
let _googleKeysFetchedAt = 0;
const GOOGLE_KEYS_TTL_MS = 60 * 60 * 1000;
...
async function verifyGoogleToken(credential) {
  ...
  return jwt.verify(credential, pubKey, {
    algorithms: ["RS256"],
...
// Pre-fetch keys at startup so the first sign-in is also fast.
...
router.post("/", authLoginLimiter, async (req, res) => {
  ...
  if (!payload.email) return res.status(401).json({ error: "Token missing email" });
