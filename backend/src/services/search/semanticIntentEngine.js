...
import { getSkillCatalog } from "./skillCatalogCache.js";
import { getNlPatternCatalog, applyDbPatternBoosts } from "./dbIntentPriors.js";
...
const FAMILY_TO_DB_KEYWORDS = {
  engineering: ["engineer", "engineering", "platform", "backend", "frontend", "devops", "infrastructure"],
  data_analytics: ["data", "analytics", "analysis", "intelligence", "scientist"],
  design_product: ["design", "product", "ux", "ui", "creative"],
  hr_people_ops: ["hr", "human resource", "people", "talent", "recruit"],
};

function buildFamilyOrFilter(targetFamilies) {
  const patterns = [...new Set(
    targetFamilies.flatMap((f) => FAMILY_TO_DB_KEYWORDS[f] || [])
  )];
  return patterns.map((p) => `role_family.ilike.%${p}%`).join(",");
}
...
async function enrichIntentCandidates(sb, normalizedQuery, filteredTokens, intentCtx, executionIntent, limitCount) {
  ...
  const familyOrFilter = buildFamilyOrFilter(targetFamilies);
  const [rolesRes, metaRes] = await Promise.all([
    familyOrFilter
      ? sb.from("roles_v2").select("id, canonical_title, role_family, hint, active").eq("active", true).or(familyOrFilter)
...
async function fallbackIntentPriorSearch(sb, normalizedQuery, intentCtx, limitCount) {
  ...
  const deptOrFilter = includeDepts.map((d) => `role_family.ilike.%${d}%`).join(",");
  const [rolesRes, metaRes] = await Promise.all([
    deptOrFilter
      ? sb.from("roles_v2").select("id, canonical_title, role_family, hint, active").eq("active", true).or(deptOrFilter)
      : sb.from("roles_v2").select("id, canonical_title, role_family, hint, active").eq("active", true),
    sb.from("role_semantic_metadata").select("role_id, keywords, search_phrases, responsibilities, work_examples, tools, output_types")
  ]);
...
export async function semanticIntentEngine(params) {
  ...
  const [{ data: metadataRows, error: metadataErr }, { skills: intentCanonicalRows, aliases: intentAliasRows }] =
    await Promise.all([
      sb
  ...
  // D: apply DB-driven nl_pattern boosts (best-effort; never throws)
  const nlCatalog = await getNlPatternCatalog().catch(() => []);
  const dbBoosted = applyDbPatternBoosts(rescored, intentInputText, nlCatalog);

  const results = dbBoosted.slice(0, Math.min(Math.max(1, Number(params.limitCount || 10)), 50));
