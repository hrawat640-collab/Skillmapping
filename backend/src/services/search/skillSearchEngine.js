...
function rerankStructuredResults(rows, normalizedSkills, options = {}) {
  ...
  const rescored = rows.map((row) => {
    ...
    // Cache required-hit result per user skill to avoid O(n²) re-computation
    // across requiredHits, niceHits, and matchedHits checks for the same row.
    const reqHitCache = new Map();
    const touchesRequired = (s) => {
      if (reqHitCache.has(s)) return reqHitCache.get(s);
      const hit = required.some((r) => userSkillTouchesCatalogSkill(s, r));
      reqHitCache.set(s, hit);
      return hit;
    };

    const requiredHits = userSkills.filter(touchesRequired);
    const niceHits = userSkills.filter(
      (s) => !touchesRequired(s) && nice.some((n) => userSkillTouchesCatalogSkill(s, n))
    );
    ...
    const userTouchesRequired = required.length === 0 || requiredHits.length > 0;
...
export async function skillSearchEngine(params) {
  ...
  const { skills: canonicalSkillRows, aliases: aliasRows } = await getSkillCatalog(sb);
